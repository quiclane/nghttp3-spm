...
build_one(){ PL="$1";ARCH="$2";SYS="$3";BD="$TMP/build-$PL-$ARCH";rm -rf "$BD";mkdir -p "$BD";cmake -S "$UP" -B "$BD" -GNinja -DCMAKE_BUILD_TYPE=Release -DBUILD_SHARED_LIBS=OFF -DCMAKE_SYSTEM_NAME=iOS -DCMAKE_OSX_SYSROOT="$SYS" -DCMAKE_OSX_ARCHITECTURES="$ARCH" -DCMAKE_C_FLAGS_RELEASE="-Oz -DNDEBUG" -DCMAKE_CXX_FLAGS_RELEASE="-Oz -DNDEBUG";cmake --build "$BD";LIB="$(find "$BD" -name 'libnghttp3.a' -type f | head -n 1)";[ -f "$LIB" ];printf '%s\n' "$LIB"; }
...
HDR="$TMP/Headers"
mkdir -p "$HDR/nghttp3"
if [ -d "$INCDIR/nghttp3" ];then rsync -a "$INCDIR/nghttp3/" "$HDR/nghttp3/";fi
...
xcodebuild -create-xcframework -library "$OUT/libnghttp3_ios_arm64.a" -headers "$HDR" -library "$OUT/libnghttp3_sim_universal.a" -headers "$HDR" -output "$OUT/NGHTTP3.xcframework"
